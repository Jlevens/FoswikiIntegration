...
$rule->skip_dirs(".git")->file->name('DEPENDENCIES'); # OK

# iterator interface
my $next = $rule->iter( 'distro' );
my $dinfo = {};
my $mcpan  = MetaCPAN::Client->new();

while ( defined( my $file = $next->() ) ) {
    checkNode( $file, $dinfo );
}

#my $dinfo = path('distro')->visit( \&checkNode, { recurse => 1 } );
...
for my $type (@types) {
    ...
    for my $dname (@dnames) {
        ...
        for my $ext (@exts) {
            ...
            my $info = sprintf "%s | %s", $ext, join(' | ', $otype, $odname ); # @{$dinfo->{deps}{$type}{$dname}{$ext}} );
                            
            push @changes,
                sprintf "| $change | %s | %s | $info |",  $type, $dname  if $change ne '';
            printf     ("| $change | %s | %s | $info |\n", $ptype, $pdname);
            ($ptype, $pdname) = ('^', '^');
        }
#        print "\n";
    }
...
sub checkNode {
    ...
    my @dirs = File::Spec->splitdir( $node );
    ...
    my @deps = -e $node ? path($node)->lines( { chomp => 1 } ) : ();
    my $only = '';
    for my $dep (@deps) {
        next unless $dep;
